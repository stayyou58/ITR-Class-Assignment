...
public class Ride2023 {
	...
	public Integer findRide(Digraph g, Bag<Integer> friendLocations, Integer yourLocation, Integer destination) {
		...
		int []hasPath = new int[friendLocations.size()];
		int n = 0;
		BreadthFirstDirectedPaths bfs = new BreadthFirstDirectedPaths(g, friendLocations);
		int shortest = bfs.distTo(destination); // 是否有朋友到destination
		
		if(shortest>0){ //判斷是否有最短路徑
			for(int friend : friendLocations) { //每位朋友做檢查
				// System.out.println(friend);
				bfs = new BreadthFirstDirectedPaths(g, friend);
				int disFriendToDes = bfs.distTo(destination); //該朋友到destination的最短距離

				if(disFriendToDes>0){ //有到目的地則加入一array
					hasPath[n++]=friend;
				}
			}
			for(int f : hasPath){ //判斷是否有經過yourlocation
				bfs = new BreadthFirstDirectedPaths(g, f);
				Iterable<Integer> shortestPath = bfs.pathTo(destination);
				for(int i : shortestPath){
					if(i==yourLocation){ //有則回傳
						return f;
					}
				}
			}
